...
$.sysop = {
    ...
    popup: {
        ...
        drag: function (tit, e) {//拖拽
            var $box = $(tit).parent().parent(), offset = $box.offset(), x = e.clientX - offset.left,
                y = e.clientY - offset.top, maxX = $(window).width() - $box.width() - 2;
            ...
            $(tit).bind('mouseup.popup', function () {
                $(document).unbind("mousemove.popup").unbind("mouseup.popup");
            });
        },
        ...
        autoTip: function (cont, fn) {
            ...
            $('#popAutoTips').show().delay(1000).hide(100, function () {
                fn && fn();
            });
        ...
        confirm: function (tit, cont, css, callback, isCache, callback2) {//弹出确认框，参数css可省略设置
            ...
            if (!$('#popConfirm').length) {
                ...
                $('#popConfirm').html('<dl><dt>' + tit + '<a class="close" href="#popConfirm"></a></dt><dd><div class="cont"><div class="cont_t">' + cont + '</div><div class="btn"><input type="button" alt="#popConfirm" class="ok close" value="确认" /> <input type="button" alt="#popConfirm" class="close" value="取消" /></div></div></dd></dl>')
                    ...
                    callback && callback.call($('#popConfirm')[0])
    ...
    tipClick: {//悬停弹出TIP
        ...
        init: function (fn) {
            $('.tip-click').live('click', function () {
                $.sysop.tipClick.show(this);
                return false;
            });
            $(document).click(function (e) {//记录
    ...
    pagination: {
        ...
        init: function (pageBox, current, total, disNum, fn) {//初始化分页
            ...
            var cont = $pageBox.find('.list'), len = total, num = current - 1, reObj = this,
                mid = Math.floor(disNum / 2);
            this.total = total;
            ...
            pageAllLink.click(function () {
                self.toPage.call(this, undefined, self, fn);
            });//绑定点击
            if (num == 0) {
                $pageBox.find('.first,.prev').hide();
            }
            if (num == len - 1) {
                $pageBox.find('.last,.next').hide();
            }
        },
    ...
    autoComplete: {
        ...
        init: function (input, url, key, enterTarget) {//常规初始化----enterTarget会引起其两次提交？@8.23
            ...
            var textKeyTip = $input.next(), sWidth = parseInt($input.css('width')),
                sHeight = parseInt($input.css('height'));
        ...
        selectBox: function (select, param) {//初始化selectBox
            ...
            var _this = this, url = param.url, value = param.value, name = param.name, enterTarget = param.enterTarget,
                fn = param.fn;//fn为select的option数据加载完调用
            ...
            var handlerData = function (data, name) {//处理数据
                ...
                var textKey = select.prev(), textKeyTip = select.next(), sWidth = parseInt(select.css('width')),
                    sHeight = parseInt(select.css('height'));
                ...
                textKey.blur(function () {//blur事件并赋值
                    ...
                    select.find('option').each(function () {//赋值select
                        if ($(this).html() == self.value) {
                            select.val($(this).val()), hasVal = true, select.change();
                            return false;
                        }//让绑定事件触发;
                    });
        ...
        keyupEvent: function (input, textKeyTip, url, key, data, enterTarget) {//共用方法
            ...
            var handlerJSON = function (json) {//处理JSON函数
                ...
                $.each(json, function () {
                    ...
                    if (!data || (data && item.toLowerCase().indexOf(input.val().toLowerCase()) == 0)) {
                        textKeyTip.append('<li>' + item + '</li>');
                    }
                });
    ...
    validation: {
        ...
        test: {
            IP: function (str) {
                return /^((\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.){3}(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/.test($.trim(str));
            },
        ...
        myCheck: {//==============验证3（自定义）
            ...
            unique: function (input, url1, url2) {//检测名字唯一性
                var self = this.parent, $input = $(self.form).find(input), tipPar = $input.closest('td'),
                    oriVal = $input.val(), url2 = url2 || '';
        ...
        init: function (form, hidCorTip, fn) {//验证表单初始化
            ...
            $form.find(':input').bind({//验证事件
                'change': function () {
                    self.delMsg(this);
                    self.valid.call(this, self);
                },//前一版本为$(this).focus().blur()@9.28;现改回，因其有可能触发多次blur事件@10.12
                'blur': function () {
                    self.valid.call(this, self);
                },
                'focus': function () {
                    self.delMsg(this);
                },//把这功能放到change里面去@11.9//重新放开@12.22，因为搜索提示鼠标选择后没有去掉错误状态
                'check': function () {
                    self.valid.call(this, self);
                },//当一行表单focusout时触发
                'checkSubmit': function () {
                    self.valid.call(this, self, true);
                    fn && fn();
                }//当整个表单提交时触发
            });
            //表单保存按钮的可用性控制
    ...
    menu: {
        tab: function (box, n, callback) {//tab菜单
            ...
            var $box = $(box), $titList = $box.children('.tit').children('li'),
                $contList = $box.children('.cont').children('li'), _this = this;
    ...
    kit: {
        ...
        selectLoad: function (select, param) {//{url,value,name,relInput,fn}
            var relStr = param.relInput ? $(param.relInput).val() : '', str = $(select).find('option').html(),
                s = '<option value="0">' + (str || '请选择') + '</option>';
